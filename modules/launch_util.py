...
def run(command, desc=None, errdesc=None, custom_env=None, live: bool = default_command_live) -> str:
    ...
    # Use list form to avoid shell injection; split string commands safely
...
def run_pip(command, desc=None, live=default_command_live):
    try:
        index_url_line = ['--index-url', index_url] if index_url != '' else []
        import shlex
        pip_args = shlex.split(command)
        full_args = [python, '-m', 'pip'] + pip_args + ['--prefer-binary'] + index_url_line
        return run(full_args, desc=f"Installing {desc}",
